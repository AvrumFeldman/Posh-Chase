...
$dates = [regex]::Match($text,"(?<=Opening\/Closing Date )([0-9]{2}\/){2}[0-9]{2} - ([0-9]{2}\/){2}[0-9]{2}").value -split " - " | foreach-object {$_ | get-date}

$text | foreach-object {
    $tr_regex = [regex]::Matches($_,"([0-9]+\/[0-9]+) (.*) (-?[0-9]+\,?[0-9]*\.[0-9]+)")
    
    $account_number = [regex]::Match($_, "(?<=: )[0-9]+\b").value

    $tr_regex | ForEach-Object {
